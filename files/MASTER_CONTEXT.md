...
11. **Registration creates ONLY `role='patient'`.** The Register screen hard-codes
    ...
    *Note 1: Registration + onboarding merged into single Register form. Patient
    personal info (name, DOB, gender, contact, address, consent) captured
    at signup. Registration requires two separate consents: Terms and Conditions AND
    the RA 10173 Data Privacy Policy. Each has its own checkbox, gated on
    opening its respective in-app modal — each checkbox is disabled until
    its document has been opened, ensuring informed consent. Both must be
    checked to submit registration. A single acceptance timestamp
    (accepted_privacy_at) records agreement to both.
    Post-registration profile allows editing only contact and
    address; other fields require clinic administrator intervention to prevent
    identity spoofing via profile edits after RA 10173 consent grant.*
    *Note 2: Registration DOB validator enforces hard 13+ age minimum. Existing
    accounts with under-13 DOB grandfathered (no migration). Applies to
    new registrations only.*
12. **Mobile is read-only for staff.** All staff roles (receptionist, department_staff,
    medical_specialist) on mobile see scoped read views only and cannot make state changes.
    Specifically, the receptionist cannot mark queue entries as arrived or approve/reject
    documents on mobile. All state changes, queue status transitions, and document approvals/rejections
    must happen on the web portal. Staff do NOT enter new patients, new lab values, or new policies
    on mobile — those are web-portal actions. As of 2026-06-22, the receptionist mobile screen does
    not surface the queue tab — queue management is owned by the web portal per joint
    architectural decision with the web team. The queue state, methods, and Realtime subscription
    are preserved in the receptionist provider for potential re-enabling or for derived UI use.
    *Note: Constraint #12 is currently under revision (Phases R1-R5 in progress) to implement the receptionist write-capable workstation on mobile. Phase R1 establishes the read-only foundation; Phase R2 implements Approve & Route; Phase R3 implements Rejection w/ Reason Presets.*
13. **Registration email OTP verification.** New patient accounts must complete
    6-digit email OTP verification before account activation. SAME OTP
    infrastructure reused for email address changes on Profile screen —
    new email must verify OTP before update. Codes hashed at rest, 10 min
    lifetime, rate limited. Notification email sent to previous address
    after successful email change.
14. **Session inactivity timeout.** Authenticated sessions terminate after inactivity: patient 20 min, staff (receptionist/department/specialist) 10 min. Live Triage Queue screen exempts patient timeout while viewing. Termination clears Supabase session, redirects to login, shows "You were logged out due to inactivity" toast. Foreground/background transitions honor timeout — apps resumed after timeout period force re-login before session restore.
15. **Password Reset flow.** Password reset uses Edge Function `request-password-reset` with server-side role check and uniform response. Staff and unknown emails silently succeed (no email sent) to prevent account enumeration. Client receives identical response for all inputs. redirectTo redirects to the web portal's `https://www.klinikaid.com/reset-password` page.
...
- [x] **Phase 8 — Staff mode (read-mostly) — PASS date: 2026-06-20**
- [x] Phase 9 — Testing, hardening & release — PASS date: 2026-06-21
