...
13. **Registration email verification.** New patient accounts must complete a 6-digit email OTP verification before the account is activated. Codes are 10 minutes lifetime, max 5 verify attempts per code, max 3 send attempts per email per hour, 60s resend cooldown. Accounts created before this feature shipped are grandfathered as verified via a one-time migration. Staff accounts are admin-created via the web portal and do NOT go through this flow.
14. **Session inactivity timeout.** Authenticated sessions terminate after inactivity: patient 20 min, staff (receptionist/department/specialist) 15 min. Live Triage Queue screen exempts patient timeout while viewing. Termination clears Supabase session, redirects to login, shows "You were logged out due to inactivity" toast. Foreground/background transitions honor timeout — apps resumed after timeout period force re-login before session restore.
