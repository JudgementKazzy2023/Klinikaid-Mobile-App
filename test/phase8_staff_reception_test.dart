...
void main() {
  ...
  group('Phase 8: Receptionist Portal Integration & Verification Chain', () {
    late String patientRecordId;
    late String docId;
    late int queueId;

    test('Data setup and retrieve checks', () async {
      ...
      final approvedDocId = generateUuid();
      await client.from('documents').insert({
        'id': approvedDocId,
        'patient_id': patientRecordId,
        'uploader_id': patientUid,
        'file_name': 'approved_report.pdf',
        'file_path': 'uploads/approved_report.pdf',
        'file_type': 'pdf',
        'status': 'approved',
      ...
      final rejectedDocId = generateUuid();
      await client.from('documents').insert({
        'id': rejectedDocId,
        'patient_id': patientRecordId,
        'uploader_id': patientUid,
        'file_name': 'rejected_receipt.pdf',
        'file_path': 'uploads/rejected_receipt.pdf',
        'file_type': 'pdf',
        'status': 'rejected',
        'rejection_reason': 'Invalid patient name',
        'updated_at': DateTime.now().toUtc().toIso8601String(),
      });
    ...
    test('RLS permits receptionist to mark queue arrived (web portal action; not exposed on mobile UI)', () async {
      // Queue status transitions are intentionally not exposed in the mobile reception UI per scope decision 2026-06-22.
    ...
    test('RLS permits receptionist to approve documents (web portal action; not exposed on mobile UI)', () async {
      // Document approval is intentionally not exposed in the mobile reception UI per scope decision 2026-06-22.
      // This test continues to verify that the underlying RLS policy permits the action, which is correct since the same policy serves the web portal.
      final client = SupabaseService.client;
      await client.auth.signInWithPassword(email: receptionistEmail, password: password);
    ...
    test('RLS permits receptionist to reject documents with reason (web portal action; not exposed on mobile UI)', () async {
      // Document rejection is intentionally not exposed in the mobile reception UI per scope decision 2026-06-22.
      // This test continues to verify that the underlying RLS policy permits the action, which is correct since the same policy serves the web portal.
      final client = SupabaseService.client;
      await client.auth.signInWithPassword(email: receptionistEmail, password: password);
