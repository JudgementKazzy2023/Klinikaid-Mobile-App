...
void main() {
  ...
  test('Phase 8: Department Staff Portal RLS & Data Isolation Verification', () async {
    ...
    final imagingStaffUid = client.auth.currentUser!.id;
    await Future.delayed(const Duration(milliseconds: 1500));
    await client.auth.signOut();
    ...
    final labQueueId = labQueueResponse['id'] as int;

    final imgQueueResponse = await client.from('patient_queue').insert({
      'patient_id': patientRecordId,
      'department': 'imaging',
      'status': 'waiting',
      'priority_level': 'urgent',
    }).select().single();
    final imgQueueId = imgQueueResponse['id'] as int;
