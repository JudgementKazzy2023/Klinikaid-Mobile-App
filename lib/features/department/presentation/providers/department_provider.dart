...
class DepartmentProvider extends ChangeNotifier {
  ...
  Future<void> loadDashboard() async {
    ...
    try {
      final queueFuture = _repo.getDailyQueue();
      final recordsFuture = _repo.getRecordsHistory();
      ...
      // Subscribe to department queue updates in real-time
      subscribeQueue();
    } catch (e) {
      _isLoading = false;
  ...
  Future<void> quietFetchQueue() async {
    try {
      final queue = await _repo.getDailyQueue();
      _queueEntries = queue;
      notifyListeners();
    } catch (e) {
      // ignore: avoid_print
